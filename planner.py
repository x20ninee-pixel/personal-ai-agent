...
from recurring import ensure_recurring_tasks_for_today
from schedule import get_events_for_date, events_overlap
from timezone_service import get_user_tzinfo, now_for_user, parse_user_datetime
...
def get_today_tasks(user_id):
    ...
    ensure_recurring_tasks_for_today(user_id)

    today = now_for_user(user_id).date().isoformat()
...
def get_deadline(task, user_id=None):
...
def get_deadline_status(
    ...
    deadline = get_deadline(task, user_id=user_id)

    if not deadline:
        return None

    now = now_for_user(user_id) if user_id is not None else datetime.now(TZ)
...
def priority_score(task, user_id=None):
...
def find_schedule_conflicts(user_id, start, end, candidate_events=None):
    if candidate_events is None:
        candidate_events = []

    conflicts = []
    for _, event_start, event_end in get_events_for_date(user_id, start.date()):
        if events_overlap(start, end, event_start, event_end):
            conflicts.append((event_start, event_end))

    for event_start, event_end in candidate_events:
        if events_overlap(start, end, event_start, event_end):
            conflicts.append((event_start, event_end))
...
def build_smart_plan(user_id):
    ...
    message = (
        "🧠 SMART DAILY PLAN\n\n"
        f"📅 "
        f"{now_for_user(user_id).strftime('%d.%m.%Y')}\n"
        f"⏰ Hozir: "
        f"{now_for_user(user_id).strftime('%H:%M')}\n\n"
    )
